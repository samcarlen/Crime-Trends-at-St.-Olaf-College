...
# Plotting number of assaults in student housing by year
...
# Plotting assault on campus (that took place outside of student housing) by year)
...
summary(r4a)
ggplot(disc.housing,aes(x=Survey.year,y=Drug.law.violations))+geom_point(alpha=1,size=2)+theme(panel.background = element_rect("lavenderblush2"))+geom_smooth(method="lm",se=FALSE,size=1,color="black")
